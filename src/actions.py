    ...
    try:
        with TemporaryDirectory() as tmpdir:
            ...
            file = functions_api._cognite_client.files.upload(
                zip_path, name=file_name, external_id=file_name, overwrite=True)
...
def try_delete_function(functions_api, external_id):
    try:
        function = functions_api.retrieve(external_id=external_id)
        print(
            f"Found existing function {external_id}. Deleting ...", flush=True)
...
def create_and_wait_for_deployment(functions_api, name, external_id, file_id):
    ...
    print(
        f"Will create function {external_id}. With api key: {api_key is not None}", flush=True)
    function = functions_api.create(
        name=name, external_id=external_id, file_id=file_id, api_key=api_key)
    print(
        f"Created function {external_id}. Waiting for deployment ...", flush=True)
    wait_time_seconds = 600  # 10 minutes
    deployed = await_function_deployment(
        functions_api, function.external_id, wait_time_seconds)
    if not deployed:
        print(
            f"Function {external_id} did not deploy within {wait_time_seconds} seconds.", flush=True)
        raise FunctionDeployTimeout(
            f"Function {external_id} did not deploy within {wait_time_seconds} seconds.")
...
def handle_push(functions_api):
    ...
    file_name = function_name.replace(
        "/", "_")+".zip"  # / not allowed in file names

    # Upload file
    file_id = zip_and_upload_folder(functions_api, '.', file_name)

    try_delete_function(functions_api, external_id)  # Delete old one first
    create_and_wait_for_deployment(
        functions_api, function_name, external_id, file_id)  # Upload new
    ...
    function = create_and_wait_for_deployment(
        functions_api, function_name_latest, external_id_latest, file_id)
    try_delete_function(functions_api, external_id)

    print(
        f"Successfully created and deployed function {external_id} with id {function.id}", flush=True)
...
def handle_pull_request(functions_api):
    ...
    file_id = zip_and_upload_folder(functions_api, '.', file_name)
    function = create_and_wait_for_deployment(
        functions_api, function_name, external_id, file_id)

    print(
        f"Successfully created and deployed PR function {external_id} with id {function.id}", flush=True)
